...
**This repository:** store the canonical file at `.agents/skills/<slug>/SKILL.md` and add a symlink `.claude/skills/<slug> -> ../../.agents/skills/<slug>` so Claude Code and Cursor both discover it. `scripts/sync-from-canonical.sh` excludes `.agents/`, so skills here stay in the Xcode mirror.
