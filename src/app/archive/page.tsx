...
export default function ArchivePage() {
  ...
  useEffect(() => {
    (async () => {
      try {
        const res = await fetch(`/api/entries?parentId=${DEMO_PARENT_ID}`);
    ...
    <main className="min-h-screen bg-warm px-6 py-16">
      <div className="max-w-xl mx-auto space-y-8">
        ...
        <div className="flex items-center justify-between">
          <button onClick={() => router.push('/')} className="text-sm text-muted hover:text-navy transition">
            ← Back
          </button>
          <h1 className="text-lg font-semibold text-navy tracking-tight">Your Archive</h1>
          <button
            onClick={() => router.push('/capture')}
            className="text-sm font-semibold text-gold hover:opacity-80 transition"
          >
            + New entry
          </button>
        </div>
            ...
            {entries.map((e) => (
              ...
              >
                ...
                <div className="flex flex-wrap gap-2 items-center">
                  <span className={`text-xs px-2 py-0.5 rounded-sm border font-medium ${DOMAIN_COLORS[e.domain] ?? 'bg-gray-50 text-gray-600 border-gray-200'}`}>
                    {e.domain}
                  </span>
