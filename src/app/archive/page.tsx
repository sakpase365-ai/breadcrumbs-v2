...
export default function ArchivePage() {
    ...
    (async () => {
      try {
        ...
        const url = searchQuery
          ? `/api/entries?q=${encodeURIComponent(searchQuery)}`
          : '/api/entries';
  ...
  function renderEntryCard(e: EntryCard) {
    ...
    <main className="min-h-screen bg-background">
      ...
      <div className="sticky top-0 z-10 bg-background/95 backdrop-blur-sm border-b border-border/40">
        ...
        <form
          ...
          className="max-w-xl mx-auto px-5 pb-2 flex items-center gap-2"
        >
          <input
            type="search"
            value={searchInput}
      ...
      <div className="max-w-xl mx-auto px-5 py-8 pb-28 space-y-10">
        ...
        {!loading && !searchQuery && filtered.length === 0 && entries.length > 0 && (
        ...
        {!loading && searchQuery && (
          <div className="space-y-4">
            <p className="text-xs text-muted-foreground/40">
              {entries.length} {entries.length === 1 ? 'result' : 'results'} for &ldquo;{searchQuery}&rdquo;
            </p>
            {entries.length === 0 && (
              <p className="py-12 text-center text-muted-foreground/40 text-sm">No breadcrumbs match.</p>
            )}
        ...
        {/* Grouped entries — default view */}
        {!loading && !searchQuery && grouped.map(([month, monthEntries], gi) => (
          <motion.section
            key={month}
          ...
          >
            ...
            <div className="space-y-3">
              {monthEntries.map((e) => renderEntryCard(e))}
            </div>
          </motion.section>
        ))}
