...
export default function CreatorDashboard() {
  ...
  return <DashboardLayout>
      ...
      {/* Recipients */}
      <div className="mb-6 rounded-xl bg-black/40 backdrop-blur-sm border border-white/10 p-4 sm:p-6">
        <div className="flex items-center justify-between mb-4">
          <h2 className="font-serif text-lg font-medium text-white">Recipients</h2>
        ...
        {recipients.length === 0 ? (
          <Link to="/creator/recipients">
            <div className="flex items-center justify-center py-6 border border-dashed border-white/20 rounded-lg hover:border-white/40 transition-colors cursor-pointer">
              <Plus className="w-5 h-5 text-white/40 mr-2" />
              <span className="text-white/60">Add your first recipient</span>
            </div>
          </Link>
        ) : (
          <div className="flex flex-wrap gap-3">
            {recipients.map(r => (
              <button
                key={r.id}
                onClick={() => handleRecipientFilter(r.id)}
            ...
            {selectedRecipient !== "all" && (
              <button
                ...
                className="flex items-center gap-2 px-4 py-2.5 rounded-lg border border-white/10 bg-white/5 text-white/60 hover:bg-white/10 hover:text-white transition-all text-sm"
              >
                Show All
              </button>
            )}
